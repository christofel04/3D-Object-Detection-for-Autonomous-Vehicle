    ...
    def generate_anchors(self, grid_sizes):
        ...
        for grid_size, anchor_size, anchor_rotation, anchor_height, align_center in zip(
            ...
            x_shifts = torch.arange(
                self.anchor_range[0] + x_offset, self.anchor_range[3] + 1e-5, step=x_stride, dtype=torch.np.float32,
            ).cuda()
            y_shifts = torch.arange(
                self.anchor_range[1] + y_offset, self.anchor_range[4] + 1e-5, step=y_stride, dtype=torch.np.float32,
            ).cuda()
