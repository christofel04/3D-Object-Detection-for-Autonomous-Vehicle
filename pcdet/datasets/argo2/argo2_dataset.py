...
def process_and_save_frame(frame_path, frame_anno, ts2idx, segname, output_dir, save_bin):
    ...
    if frame_anno is not None:
        ...
        annos['name'] = cat
        annos['truncated'] = np.zeros(num_obj, dtype=float64)
        annos['occluded'] = np.zeros(num_obj, dtype=np.int64)
        annos['alpha'] = -10 * np.ones(num_obj, dtype=float64)
        annos['dimensions'] = lwh.numpy().astype(float64)
        annos['location'] = xyz.numpy().astype(float64)
        annos['rotation_y'] = yaw.numpy().astype(float64)
