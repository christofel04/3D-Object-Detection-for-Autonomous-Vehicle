...
class DataBaseSampler(object):
    ...
    def sample_with_fixed_number(self, class_name, sample_group):
        ...
        print( "Sampling ground truth bounding box for class : " + str( class_name ) + " with number of bounding box : " + str( len(self.db_infos[class_name]) ))
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            if self.use_shared_memory:
                ...
                if obj_points.shape[0] != info['num_points_in_gt']:
                    obj_points = np.fromfile(str(file_path), dtype=float64).reshape(-1, self.sampler_cfg.NUM_POINT_FEATURES)
